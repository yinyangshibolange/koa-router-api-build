      ...
      if (
        ...
        if (Array.isArray(indexDataDefault)) {
          ...
          this.apis.push({
            ...
            handler:
              indexDataDefault.handler || function (ctx: RouterContext) {},
            ...indexDataDefault,
          });
...
export function genApisFile() {
  ...
  function run(argv) {
    ...
    function startGen() {
      ...
      apiBuild.genApis().then(async (apis) => {
        let fileString = `import Router from "koa-joi-router"\n`;
        apis.forEach((api) => {
          const moduleName = (api.path as string)
            .replace(/\.\w+$/, "")
            .replace(/\//g, "_");
          const dir = apisPath.replace(
            new RegExp(`^${path.parse(argv.out).dir}`),
            ""
          );
          if (api.importType === "object") {
            fileString += `import ${moduleName} from ".${dir}/${api.import.replace(
              /\.ts$/,
              ".js"
            )}"\n`;
          } else if (api.importType === "*") {
            fileString += `import * as ${moduleName} from ".${dir}/${api.import.replace(
              /\.ts$/,
              ".js"
            )}"\n`;
